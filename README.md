...
You can use prebuilt Docker Image `ghcr.io/rebel028/react2shell-scanner` or build it locally with Dockerfile provided.
...
```shell
docker pull ghcr.io/rebel028/react2shell-scanner
```
...
```shell
docker run --rm ghcr.io/rebel028/react2shell-scanner -u https://example.com
```
...
```shell
docker run --rm -v $PWD/hosts.txt:/opt/react2shell/hosts.txt:ro ghcr.io/rebel028/react2shell-scanner -l hosts.txt
```
...
docker run --rm \
  ...
  ghcr.io/rebel028/react2shell-scanner -l hosts.txt -t 20 -o results.json
```
etc.
